...
const STATUS_COLORS = {
  boarding: 'bg-[#1C4587]/20 text-[#1C4587]',
  'final call': 'bg-[#1C4587]/20 text-[#FFFFFF]',
  delayed: 'bg-[#D4AF37]/20 text-[#333333]',
  on_time: 'bg-[#98FFD9]/20 text-[#1C4587]',
  scheduled: 'bg-[#98FFD9]/20 text-[#1C4587]',
  cancelled: 'bg-red-500/20 text-red-400',
  landed: 'bg-slate-500/20 text-slate-500',
  arrived: 'bg-[#98FFD9]/20 text-[#1C4587]',
  departed: 'bg-[#98FFD9]/20 text-[#1C4587]',
}
...
export default function FlightsPage() {
  ...
  return (
    <div className="min-h-full bg-[#F4F7F9] flex flex-col">
      {/* Header */}
      ...
      <div className="px-5 mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-[#D1D5DB]
                       rounded-xl text-sm text-[#333333] placeholder-[#333333]
                       focus:outline-none focus:ring-2 focus:ring-[#1C4587]/30 focus:border-[#1C4587]/50"
          />
        </div>
      </div>
      ...
      {/* Direction Toggle */}
      <div className="flex mx-5 mb-5 bg-[#E0E4E8] rounded-xl p-1">
        <button
          onClick={() => setDirection('departure')}
          className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
            direction === 'departure'
              ? 'bg-[#1C4587] text-white'
              : 'bg-[#E0E4E8] text-[#333333]'
          }`}
        >
          Departures
        </button>
      ...
      <div className="flex-1 px-5 space-y-3 pb-24 overflow-y-auto">
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="animate-spin w-8 h-8 border-2 border-[#1C4587] border-t-transparent rounded-full" />
          </div>
        ) : filtered.length === 0 ? (
...
function FlightCard({ flight, showAddButton, onAdd }) {
  ...
  return (
    <div className="bg-white border border-[#E0E4E8] rounded-2xl p-4 hover:bg-white transition-colors">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-[#333333] text-lg font-bold">{flight.origin_code || '--'}</span>
          <svg className="w-5 h-5 text-[#555555]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
          </svg>
          <span className="text-[#333333] text-lg font-bold">{flight.destination_code || '--'}</span>
        </div>
        <span className="text-[#555555] text-xs">Gate {flight.gate || '--'}</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-[#555555]">
          ...
          {isDelayed && flight.estimated_at ? (
            ...
            <span className="line-through text-[#555555] mr-1">{formatTime(flight.scheduled_at)}</span>
              <span className="text-red-400">{formatTime(flight.estimated_at)}</span>
            </span>
          ) : (
            <span className="text-xs text-[#555555]">{formatTime(flight.scheduled_at)}</span>
          )}
        </div>
        {showAddButton ? (
          <button onClick={onAdd}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1C4587] hover:bg-[#1C4587]/90 border border-[#1C4587]/30 rounded-lg transition-colors">
